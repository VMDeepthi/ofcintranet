    ...
    db.query(check_query, check_value, (err, result) => {
        if (err) {
            ...
            if (result.length === 0) {
                ...
                return res.status(406).json('check your reporting stucture and change users which is not belongs to other reporting structure and must not add reporting head in users')
            }
        }
    })
}
        ...
        db.query(check_query, check_value, async (err, result) => {
            if (err) {
                ...
                if (result.length === 0) {
                    ...
                    return res.status(500).json('check your reporting stucture and change users which is not belongs to other reporting structure and must not add reporting head in users')
                }
...
export const deletereportingstructure = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json('reporting structure deleted successfully')
    }
    catch {
        return res.status(500).json('error occured!')
    }
